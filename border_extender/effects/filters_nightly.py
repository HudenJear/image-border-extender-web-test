...
from .filter_utils import apply_vignette
from .filter_utils import apply_lut
from .filter_utils import make_lut_filter
...
def _filter_film_kodak_5219(img: Image.Image, strength: float) -> Image.Image:
    ...
    arr_t = _apply_film_grain(arr_t, s, mid=0.5, sigma_base=0.06, sigma_slope=0.08, cw=[0.95, 1.00, 1.18], downsample=1)
    arr_t = apply_vignette(arr_t, s, inner=0.2, outer=0.8, amount=0.45)
...
def _filter_film_kodak_e100(img: Image.Image, strength: float) -> Image.Image:
    ...
    arr_t = _apply_film_grain(arr_t, s, mid=0.5, sigma_base=0.008, sigma_slope=0.015, cw=[0.98, 1.00, 1.05], downsample=1)
    return float_rgb_to_image(arr_t)
...
def _filter_film_fuji_c100(img: Image.Image, strength: float) -> Image.Image:
    ...
    arr_t = _apply_film_grain(arr_t, s, mid=0.55, sigma_base=0.016, sigma_slope=0.024, cw=[0.96, 1.00, 1.08], downsample=1)
...
def _filter_film_kodak_g200(img: Image.Image, strength: float) -> Image.Image:
    ...
    arr_t = _apply_film_grain(arr_t, s, mid=0.5, sigma_base=0.02, sigma_slope=0.03, cw=[1.02, 1.00, 0.98], downsample=1)
    return float_rgb_to_image(arr_t)
...
def _filter_lut01(img: Image.Image, strength: float) -> Image.Image:
    ...
    if s <= 0:
        return img
    return apply_lut(img, s, 'cubes/Titanium_Cinematic_01.cube')
